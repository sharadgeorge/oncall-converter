...
class RadiologyConfig(DepartmentConfig):
    ...
    def validate_and_configure(self, files, month, year):
        ...
        if 'WORK SCHEDULE' in wb_work.sheetnames:
            ws_work = wb_work['WORK SCHEDULE']
            print(f"  ✓ Found 'WORK SCHEDULE' sheet")
        else:
            print(f"  Available sheets: {wb_work.sheetnames}")
            sheet_name = input("  Enter the Work Schedule sheet name: ").strip()
        ...
        if 'Sheet1' in wb_oncall.sheetnames:
            ws_oncall = wb_oncall['Sheet1']
            print(f"  ✓ Found 'Sheet1' sheet")
        else:
            print(f"  Available sheets: {wb_oncall.sheetnames}")
            sheet_name = input("  Enter the OnCall Schedule sheet name: ").strip()
        ...
        if detected_month:
            ...
            # Prompt for month/year if not detected
            month_input = input("  Enter month (name or number) and year: ").strip()
            month = None
