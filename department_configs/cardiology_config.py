...
class CardiologyConfig(DepartmentConfig):
    ...
    def get_default_row_config(self):
        ...
        Returns:
            dict: Default row numbers for each team
        """
        return {
    ...
    def get_default_sheet_names(self):
        ...
        """
        return {
            0: 'Sheet1',  # File 1: Rotation Schedule (Teams 94 and 123)
    ...
    def validate_and_configure(self, files, month, year, custom_config=None, selected_sheets=None):
        ...
        # Select sheet for rotation file
        if selected_sheets and 0 in selected_sheets:
            rotation_sheet_name = selected_sheets[0]
            if rotation_sheet_name in wb_rotation.sheetnames:
                print(f"  ✓ Using selected sheet: '{rotation_sheet_name}'")
                ws_rotation = wb_rotation[rotation_sheet_name]
            else:
                print(f"  ⚠ Selected sheet not found, using: '{wb_rotation.sheetnames[0]}'")
                ws_rotation = wb_rotation[wb_rotation.sheetnames[0]]
        else:
            # Use first sheet as default
            ws_rotation = wb_rotation[wb_rotation.sheetnames[0]]
            print(f"  ✓ Using sheet: '{wb_rotation.sheetnames[0]}'")
        ...
        if custom_config:
            ...
            team94_row = custom_config.get('Team 94', 29)
        ...
        workbooks = {
            ...
            'team8': wb_team8,
            'ws_rotation': ws_rotation,  # Selected worksheet
            'ws_team8': ws_team8,         # Selected worksheet
    ...
    def _read_interventional_data(self, wb, config, month_num, year):
        ...
        # If still not found, show error and skip Team 94
        if not ws:
            print(f"\n  ⚠ ERROR: Could not find sheet for Team 94 for {month_name} {year}")
            print(f"  Expected: Sheet containing '{month_name}' or '{month_abbr}' with 'Attending'")
            print(f"\n  Available sheets in workbook:")
            for i, sheet_name in enumerate(wb.sheetnames, 1):
                print(f"    {i}. {sheet_name}")
            print(f"\n  💡 Tip: Use the sheet selector in Step 3 to choose the correct sheet")
            print(f"  ⚠ WARNING: Skipping Team 94 data...")
            return {}
    ...
    def _read_cardiology_data(self, wb, config, month_num, year):
        ...
        # Error if Attending sheet not found
        if not attending_sheet:
