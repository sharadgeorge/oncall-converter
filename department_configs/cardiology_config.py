...
class CardiologyConfig(DepartmentConfig):
    ...
    def validate_and_configure(self, files, month, year):
        ...
        if detected_month:
            ...
            if confirm in ['n', 'no']:
                ...
                try:
                    ...
                    # If month not found, use detected values
                    if month is None:
            ...
            try:
                ...
                if year_match:
                    year = int(year_match.group())
                # If month still not found, use current month as fallback
                if month is None:
        ...
        # Row configuration prompts (like Cardiology style)
        print("\nRow Configuration:")
        print("-" * 60)
        
        # Team 123 Consultant rows
        default_consultant_rows = (6, 11)
        user_input = input(f"Team 123 Consultant rows [default: {default_consultant_rows[0]}-{default_consultant_rows[1]}]: ").strip()
        if user_input and '-' in user_input:
            try:
                start, end = user_input.split('-')
                consultant_rows = (int(start), int(end))
                print(f"✓ Using rows {start}-{end} for Team 123 Consultants")
            except:
                consultant_rows = default_consultant_rows
                print(f"✓ Using default rows")
        else:
            consultant_rows = default_consultant_rows
            print(f"✓ Using default rows {default_consultant_rows[0]}-{default_consultant_rows[1]}")
        
        # Team 123 Staff/Fellows rows
        default_staff_rows = (6, 13)
        user_input = input(f"Team 123 Staff/Fellows rows [default: {default_staff_rows[0]}-{default_staff_rows[1]}]: ").strip()
        if user_input and '-' in user_input:
            try:
                start, end = user_input.split('-')
                staff_rows = (int(start), int(end))
                print(f"✓ Using rows {start}-{end} for Team 123 Staff/Fellows")
            except:
                staff_rows = default_staff_rows
                print(f"✓ Using default rows")
        else:
            staff_rows = default_staff_rows
            print(f"✓ Using default rows {default_staff_rows[0]}-{default_staff_rows[1]}")
        
        # Team 94 row
        default_team94_row = 29
        user_input = input(f"Team 94 row [default: {default_team94_row}]: ").strip()
        if user_input and user_input.isdigit():
            team94_row = int(user_input)
            print(f"✓ Using row {team94_row} for Team 94")
        else:
            team94_row = default_team94_row
            print(f"✓ Using default row {default_team94_row}")
        
        # Team 8 rows
        default_team8_rows = (12, 16)
        user_input = input(f"Team 8 rows [default: {default_team8_rows[0]}-{default_team8_rows[1]}]: ").strip()
        if user_input and '-' in user_input:
            try:
                start, end = user_input.split('-')
                team8_rows = (int(start), int(end))
                print(f"✓ Using rows {start}-{end} for Team 8")
            except:
                team8_rows = default_team8_rows
                print(f"✓ Using default rows")
        else:
            team8_rows = default_team8_rows
            print(f"✓ Using default rows {default_team8_rows[0]}-{default_team8_rows[1]}")
